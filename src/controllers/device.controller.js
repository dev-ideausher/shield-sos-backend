...
const createDevice = catchAsync(async (req, res) => {
  const acceptPassword = req.query.acceptPassword === "true";

  const password = acceptPassword
    ? req.body.password
...
const updateDevice = catchAsync(async (req, res) => {
  const data = await deviceService.updateDeviceById(
    req.params.deviceId,
    req.body
  );
  res.json({ data });
