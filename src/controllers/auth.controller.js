...
const loginUser = catchAsync(async (req, res) => {
    ...
    let checkUser = await userService.getUserByEmailOrUsername(username)
    if (!checkUser) {
        return res.status(400).json({
            status: false,
    ...
    let decryptedPassword = decryptText(checkUser.password, config.password_secret_key);
    if (decryptedPassword !== password) {
        return res.status(400).json({
            status: false,
            message: "Invalid creds. Try again..."
        })
    }
    // Generate a new custom token
    const token = jwt.sign({ id: checkUser._id, email: checkUser.email, role: checkUser.role }, config.jwtSecret, { expiresIn: config.jwtExpiry });
    let permissions = await permissionService.getPermissionByUserId(checkUser._id);
    if (checkUser.role === 'superadmin') {
        ...
        req.user = {
            ...
            permissions: {
                ...
                viewPayIn: permissions.viewPayIn,
                viewPayOut: permissions.viewPayOut
            }
        }
    } else if (checkUser.role === 'super_distributor') {
        req.user = {
            ...checkUser.toJSON(),
            permissions: {
                _id: permissions._id,
    ...
    } else if (checkUser.role === 'distributor') {
        req.user = {
            ...checkUser.toJSON(),
            permissions: {
                _id: permissions._id,
    ...
    } else if (checkUser.role === 'retailer') {
        req.user = {
            ...checkUser.toJSON(),
            permissions: {
                _id: permissions._id,
